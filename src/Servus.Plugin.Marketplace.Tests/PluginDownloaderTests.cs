...
public sealed class PluginDownloaderTests : IDisposable
{
    ...
    public async Task Download_GithubRelease_SavesDll()
    {
        ...
        var delivery = new PluginDelivery { Type = DeliveryType.GitHubRelease, Asset = "Servus.TUI.Plugin.Dtop.dll" };
    ...
    private sealed class FakeDownloadHandler(byte[] content) : HttpMessageHandler
    {
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken ct)
        {
            ...
            {
                const string json = """{"assets": [{"name": "Servus.TUI.Plugin.Dtop.dll", "browser_download_url": "https://download.example.com/Servus.TUI.Plugin.Dtop.dll"}]}""";
