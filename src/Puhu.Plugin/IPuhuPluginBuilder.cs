...
/// <summary>
/// Fluent builder for configuring a Puhu plugin — tabs, routes, actors, services, settings, themes, and notifications.
/// </summary>
public interface IPuhuPluginBuilder
{
    ...
    IPuhuPluginBuilder WithTab(string label, string route, ConsoleKey? hotKey = null);

    /// <summary>Register page routes for navigation.</summary>
    IPuhuPluginBuilder ConfigureRoutes(Action<IRouteContext> configure);

    /// <summary>Register Akka.NET actors for this plugin.</summary>
    IPuhuPluginBuilder ConfigureActors(Action<IActorContext> configure);
    ...
    IPuhuPluginBuilder WithSettings<T>(string sectionKey) where T : class, new();

    /// <summary>Register custom theme color keys.</summary>
    IPuhuPluginBuilder WithTheme(Action<IThemeContext> configure);

    /// <summary>Register notification types this plugin can emit.</summary>
    IPuhuPluginBuilder WithNotifications(Action<INotificationContext> configure);
}
