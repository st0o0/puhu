...
/// <summary>
/// Assertion helpers over a <see cref="VirtualTerminal"/>, with readable failure messages
/// that dump the screen. <see cref="WaitForTextAsync"/> polls so it tolerates the app loop
/// re-rendering on a background thread (a read mid-render just triggers another poll).
/// </summary>
public static class ScreenAssert
{
    ...
    /// <summary>Poll <paramref name="condition"/> until true, or fail after the timeout.</summary>
        ...
        int pollIntervalMs = 50)
    {
        ...
        Assert.True(condition(), $"{message} (within {timeoutMs}ms)");
    }
}
